...
public class CalcServlet extends HttpServlet {
    ...
    private int calculate(HttpServletRequest req){
        ...
        while (equation.indexOf('(') != -1){
            ...
            equation = doOperations(equation.substring(leftBorder, rightBorder + 1));
            rightBorder = equation.indexOf(')', leftBorder);
            if (rightBorder < equation.length() - 1){
                equation = equation.substring(0,leftBorder) +
                        equation.substring(leftBorder + 1, rightBorder) +
