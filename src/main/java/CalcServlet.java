...
public class CalcServlet extends HttpServlet {
    ...
    private String calc(String str) {
        ...
        for (int i=1; i<str.length(); i++) {
            ...
            if (j == str.length()) {
                if (operators.charAt(1) == ' ') tmpres = toNumber(subs);
                else tmpres = solveNPath(operators.charAt(1), tmpres, toNumber(subs));
                res = solveNPath(operators.charAt(0), res, tmpres);
                i=j;
                continue;
            }
            if (operators.charAt(1) == '*' || operators.charAt(1) == '/') {
                tmpres = solveNPath(operators.charAt(1), tmpres, toNumber(subs));
                ...
                if (str.charAt(j) == '*' || str.charAt(j) == '/') {
                    tmpres += toNumber(subs);
                    operators = operators.replace(operators.charAt(1),str.charAt(j));
                }
                else {
                    if (operators.charAt(0) == '+') res += toNumber(subs);
                    else res -= toNumber(subs);
    ...
    private int toNumber(String sn) {
        int result = 0;
        int minus = 1;
        String toNumber = sn;
        if (toNumber.charAt(0) == '-') {
            minus = -1;
            toNumber = toNumber.replace('-', '0');
        }
        for (int i=0; i<toNumber.length(); i++) {
            result = result*10 + Character.getNumericValue(toNumber.charAt(i));
        }
        return result*minus;
    }
